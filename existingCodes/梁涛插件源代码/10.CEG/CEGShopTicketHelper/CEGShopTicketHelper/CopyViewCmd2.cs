...
namespace CEGShopTicketHelper
{
    ...
    public class CopyViewCmd2 : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            //3.sourceVCI
            ViewContentInfo2 sourceVCI = new ViewContentInfo2(doc.ActiveView);
            sourceVCI.CollectTagInfos();
            sourceVCI.CollectAnnotationItems();
            sourceVCI.CollectDimensionInfos();

            //4. targetVCI
            ViewContentInfo2 targetVCI = new ViewContentInfo2(targetView);
            ...
            foreach (DimensionInfo dimInfo in sourceVCI.DimensionInfos)
            {
                ...
                foreach (Reference r in dimInfo.References)
                {
                    ...
                    //如果是上下左右四个边
                    //直接添加
                    Reference rr = sourceVCI.FindBoundaryReference(r, targetVCI);
                    //TaskDialog.Show("r", targetVCI.LeftReferences.Count().ToString() + (targetVCI.LeftReferences[0] == null).ToString());
                    if (rr != null)
                    {
                        referenceArray.Append(rr);
                        continue;
                    }
                    ...
                    if (dimInst.Category.Id.IntegerValue == (int)BuiltInCategory.OST_SpecialityEquipment)
                    {
                        ...
                        XYZ dir1 = (dimInst.Location as LocationPoint).Point - sourceVCI.ViewOrigin;
                        foreach (EmbedRebarInfo embedRebarInfo in targetVCI.EmbedRebarInfos)
                        {
                            double d2 = (embedRebarInfo.Location - targetVCI.ViewOrigin).GetLength();
                            XYZ dir2 = embedRebarInfo.Location - targetVCI.ViewOrigin;
                            //if (Math.Abs(d1 - d2) < 0.001)
                            if (dir1.IsAlmostEqualTo(dir2, 0.001))
                            {
                                //using (Transaction trans = new Transaction(doc, "2"))
                                //{
                                //    trans.Start();
                                //    TaskDialog.Show("r", d1.ToString() + "," + d2.ToString());
                                //    XYZ pt1 = (dimInst.Location as LocationPoint).Point;
                                //    XYZ pt2 = sourceVCI.ViewOrigin;
                                //    XYZ pt3 = embedRebarInfo.Location;
                                //    XYZ pt4 = targetVCI.ViewOrigin;
                                //    doc.Create.NewDetailCurve(sourceVCI.View, Line.CreateBound(pt1, pt1 + sourceVCI.View.UpDirection));
                                //    doc.Create.NewDetailCurve(sourceVCI.View, Line.CreateBound(pt2, pt2 + sourceVCI.View.UpDirection));
                                //    doc.Create.NewDetailCurve(targetVCI.View, Line.CreateBound(pt3, pt3 + targetVCI.View.UpDirection));
                                //    doc.Create.NewDetailCurve(targetVCI.View, Line.CreateBound(pt4, pt4 + targetVCI.View.UpDirection));
                                //    trans.Commit();
                                //}
                                //XYZ dir = newDimInfo.Line.Direction.CrossProduct(sourceVCI.View.ViewDirection);
                                XYZ dir = sourceVCI.View.RightDirection;
                                Reference rrr = targetVCI.GetInstanceReference(embedRebarInfo.Host, dir);
                                if (rrr != null)
                                {
                                    //TaskDialog.Show("r", "find");
                                    referenceArray.Append(rrr);
                                    continue;
                                }
                ...
                //TaskDialog.Show("r", newDimInfo.References.Size.ToString());
            ...
            using (Transaction trans = new Transaction(doc, "copyView"))
            {
                ...
                if (sourceVCI.AnnotationItemIds.Count > 0 && copyAnnotation)
                {
                    ...
                    List<ElementId> newIds1 = ElementTransformUtils.CopyElements(
                        doc.ActiveView, sourceVCI.AnnotationItemIds, targetView,
                        Transform.Identity, copyPasteOptions).ToList();

                    ////rotate
                    //double angle = XYZ.BasisX.AngleTo(doc.ActiveView.CropBox.Transform.BasisX);
                    ...
                    //    ElementTransformUtils.RotateElements(doc, newIds1, axis, angle);
                    //}
                    ...
                    XYZ pt2 = doc.GetElement(newIds1.First()).get_BoundingBox(targetView).Max
                        - targetVCI.ViewOrigin;
                    ElementTransformUtils.MoveElements(doc, newIds1, pt1 - pt2);

                }
                if (sourceVCI.ViewerItemIds.Count > 0 && copyAnnotation)
                {
                    CopyPasteOptions copyPasteOptions = new CopyPasteOptions();
                    List<ElementId> newIds2 = ElementTransformUtils.CopyElements(
                        doc.ActiveView, sourceVCI.ViewerItemIds, targetView,
                        Transform.Identity, copyPasteOptions).ToList();
                    //move elements to fix offset
                    XYZ pt3 = doc.GetElement(sourceVCI.ViewerItemIds.First()).get_BoundingBox(doc.ActiveView).Max
                        - sourceVCI.ViewOrigin;
                    XYZ pt4 = doc.GetElement(newIds2.First()).get_BoundingBox(targetView).Max
                        - targetVCI.ViewOrigin;
                    ElementTransformUtils.MoveElements(doc, newIds2, pt3 - pt4);
                }
                    ...
                    foreach (DimensionInfo dimInfo in targetVCI.DimensionInfos)
                    {
                        try
                        {
                            using (Transaction st1 = new Transaction(doc, "copy dim"))
                            {
                                ...
                                //TaskDialog.Show("r", dim.Id.IntegerValue.ToString());
