...
namespace CEGRegister
{
    ...
    public class Command10 : IExternalCommand
    {
        ...
        {
            get { return Command10._direction; }
            set { Command10._direction = value; }
        }
        private static string _distance;
        public static string Distance
        {
            get { return Command10._distance; }
            set { Command10._distance = value; }
        }
        #endregion
    ...
    System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location));
            ...
            //判断是否存在三维视图
            View3D view = Get3DJDView(doc);
            if (null == view)
            {
                message = "错误1：未找到｛3D 机电｝视图！";
        ...
        /// <param name="doc">当前文档</param>
        /// <returns>三维视图</returns>
        public View3D Get3DJDView(Document doc)
        {
            View3D view = null;
            List<Element> list = new List<Element>();
            FilteredElementCollector collector = new FilteredElementCollector(doc);
            ...
            foreach (Autodesk.Revit.DB.View3D v in list)
            {
                if (v != null && !v.IsTemplate && v.Name == "3D 机电")
                {
                    view = v as View3D;
                    break;
                }
