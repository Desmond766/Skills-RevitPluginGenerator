...
namespace CEGRegister
{
    ...
    public class Command06 : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            //载入族
            string filePath = @"C:\\ProgramData\Autodesk\Revit\Addins\2020\0PC_标注族";
            try
            {
                LoadAllFamiliesFromDirectory(uiapp, filePath);
            }
            catch (Exception ex)
            {
                TaskDialog.Show("载入族错误", ex.Message);
            }
            ...
            List<View3D> fristCollector = new FilteredElementCollector(doc).OfClass(typeof(View3D)).Cast<View3D>().ToList();
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        TaskDialog.Show("平面标注", "结束布置");
                        break;
                    }
                    else
                    {
                        TaskDialog.Show("其他错误",
                ...
                List<Element> collector = new FilteredElementCollector(doc, doc.ActiveView.Id)
                .WherePasses(boxIntersectsFilter).WherePasses(filter).OfClass(typeof(MEPCurve)).ToList();
                ...
                using (Transaction t = new Transaction(doc, "TAGHELPER"))
                {
                    ...
                    foreach (Element element in collectorToList)
                    {
                        ...
                        //底部净高
                        double netHeight = Utils.GetClearHeight(view3D, doc, point_Do);
        ...
        public void LoadAllFamiliesFromDirectory(UIApplication uiapp, string directoryPath)
        {
            // 获取当前文档
            Document doc = uiapp.ActiveUIDocument.Document;

            // 获取目录中的所有 .rfa 文件
            string[] familyFiles = Directory.GetFiles(directoryPath, "*.rfa");

            // 开启一个事务组，以便在一个事务中处理所有族文件
            using (TransactionGroup transGroup = new TransactionGroup(doc, "载入族"))
            {
                transGroup.Start();

                foreach (string familyFilePath in familyFiles)
                {
                    ...
                    if (!IsFamilyLoaded(doc, familyName))
                    {
                        ...
                        using (Transaction trans = new Transaction(doc, "Load Family"))
                        {
                            ...
                            Family family;
                            doc.LoadFamily(familyFilePath, out family);
                            //if (doc.LoadFamily(familyFilePath, out family))
                            //{
                            //    TaskDialog.Show("Revit", $"Family {familyName} loaded successfully!");
                            //}
                            //else
                            //{
                            //    TaskDialog.Show("Revit", $"Failed to load family {familyName}.");
                            //}
                    ...
                    //else
                    //{
                    //    TaskDialog.Show("Revit", $"Family {familyName} already exists in the project.");
                    //}
                }
