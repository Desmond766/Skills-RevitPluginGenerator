            ...
            while (true)
            {
                try
                {
                    ...
                    using (Transaction t = new Transaction(doc, "Add Hanger"))
                    {
                        ...
                        ref_mep = selection.PickObject(ObjectType.Element, new SelectionFilter4MEPCurve(), "点击管线进行布置");
                        ...
                        {
                            ResolverForDuct resolverForDuct = new ResolverForDuct(commandData);
                        ...
                        {
                            ResolverForPipe resolverForPipe = new ResolverForPipe(commandData);
                            resolverForPipe.Resolve(view3D, hanger, mep, location);
                        }
                        //线槽
                        else if (mep.Category.Id.IntegerValue == (int)BuiltInCategory.OST_CableTray)
                        {
                            ResolverForCableTray resolverForCableTray = new ResolverForCableTray(commandData);
