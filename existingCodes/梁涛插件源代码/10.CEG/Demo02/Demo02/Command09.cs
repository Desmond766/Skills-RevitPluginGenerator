...
namespace Demo02
{
    ...
    public class Command09 : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            FamilyInstance familyInstance1 = doc.GetElement(sel.PickObject(ObjectType.Element, new SpecializedEquipmentFilter(), "Select the first sleeve")) as FamilyInstance;
            FamilyInstance familyInstance2 = doc.GetElement(sel.PickObject(ObjectType.Element, new SpecializedEquipmentFilter(), "Select the second sleeve")) as FamilyInstance;
            FamilyInstance familyInstance3 = doc.GetElement(sel.PickObject(ObjectType.Element, new SpecializedEquipmentFilter(), "Select the steel bars to be arranged between two sleeves")) as FamilyInstance;
            FamilySymbol familySymbol = familyInstance3.Symbol;
            double famLength = 0;
            try
            {
                famLength = familyInstance1.Symbol.LookupParameter("Sleeve Length (mm)").AsDouble();
            }
            catch (Exception)
            {

                TaskDialog.Show("tip", "The family instance does not have the parameter \"Sleep Length (mm)\"");
                return Result.Failed;
            }


            UserControl03 userControl03 = new UserControl03();
            ...
            //}
            userControl03.ShowDialog();
            if (userControl03.cancel)
            {
            ...
            double insertionLength = Convert.ToDouble(userControl03.textBox.Text) / 304.8;
