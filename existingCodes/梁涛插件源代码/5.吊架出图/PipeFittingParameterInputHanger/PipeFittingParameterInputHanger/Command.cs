...
namespace PipeFittingParameterInputHanger
{
    ...
    System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location));
            ...
            using (Transaction t = new Transaction(doc, "创建吊架文字注释"))
            {
                ...
                foreach (var item in references)
                {
                    try
                    {
                        ...
                        HangerAnnotation(doc, item, userControl, view3D);
        ...
        private void HangerAnnotation(Document doc, Reference reference, UserControl1 userControl, View3D view3D)
        {
            ...
            bool includingInsulation = false;
            if (userControl.Insulation_Layer.IsChecked == true) includingInsulation = true;

            Solid solid = CreateSolidByHanger(familyInstance, false, includingInsulation);

            // 获取MepCurve信息
            List<Product8> colist = GetMepCurveInfos(doc, familyInstance, solid, intersectsFilter);
            ...
            // 获取文字注释信息
            string info = GetTextNoteInfo(familyInstance, colist, userControl);
            // 创建文字注释
            CreateTextNote(doc, info, centerP, reference);

            // 双层吊架
            if (familyInstance.Symbol.FamilyName.Contains("双层") && familyInstance.LookupParameter("上层管道底标高") != null)
            {
                Solid upSolid = CreateSolidByHanger(familyInstance, true, includingInsulation);
                List<Product8> upColist = GetMepCurveInfos(doc, familyInstance, upSolid, intersectsFilter);

                if (upColist.Count == 0)
                {
                    return;
                }
                string upInfo = GetTextNoteInfo(familyInstance, upColist, userControl);
                CreateTextNote(doc, upInfo, centerP, reference, 1);
            }
        }
        private void AddMethod(Document doc, Reference reference, UserControl1 userControl, View3D view3D)
        {
            ...
            else
            {
                ...
                else if (b3.Count() != 0 && b.Count() != 0)
                {
                    info = info.Replace("Q", "Z");
                }
        ...
        /// <param name="createDoubleLayer">是否创建第二层solid</param>
        /// <param name="includingInsulation">管道是否考虑保温层</param>
        /// <returns></returns>
        public Solid CreateSolidByHanger(FamilyInstance hanger, bool createDoubleLayer = false, bool includingInsulation = false)
        {
            Solid solid;

            XYZ centerP = (hanger.Location as LocationPoint).Point;
            if (createDoubleLayer) centerP += XYZ.BasisZ * hanger.LookupParameter("上层管道底标高").AsDouble();
            else centerP += XYZ.BasisZ * hanger.LookupParameter("b_底层管道底高").AsDouble();

            XYZ direction = hanger.GetTransform().OfVector(XYZ.BasisX);

            double length = 0;
            foreach (Parameter item in hanger.Parameters)
                if (item.Definition.Name == "c_桥架宽" || item.Definition.Name == "c_风管宽") length = item.AsDouble();

            XYZ verticalVector = direction.CrossProduct(XYZ.BasisZ);
            ...
            // 若存在保温层则多加50mm判定高度
            double high = 100 / 304.8;
            if (includingInsulation) high = 150 / 304.8;
            solid = GeometryCreationUtilities.CreateExtrusionGeometry(loops, XYZ.BasisZ, high);

            return solid;
        }
        private List<Product8> GetMepCurveInfos(Document doc, FamilyInstance hanger, Solid solid, BoundingBoxIntersectsFilter intersectsFilter)
        {
            List<Product8> colist = new List<Product8>();
            ...
            foreach (Element mep in intersectingElements)
            {
                ...
                double spacing = lowHeight - hangerGC - hanger.LookupParameter("b_底层管道底高").AsDouble();
                if (mep.Category.Id.IntegerValue == (int)BuiltInCategory.OST_DuctCurves)
                {
                    wide = mep.get_Parameter(BuiltInParameter.RBS_CURVE_WIDTH_PARAM).AsDouble();
                    height = mep.get_Parameter(BuiltInParameter.RBS_CURVE_HEIGHT_PARAM).AsDouble();
        ...
        private string GetTextNoteInfo(FamilyInstance hanger, List<Product8> colist, UserControl1 userControl)
        {
            string info = "";
            ...
            {
                var curv = colist.FirstOrDefault();
                info = "F-" + curv.MEPwidth.ToString() + "*" + curv.MEPheight.ToString() + "-" + hanger.LookupParameter("T/TL").AsString();
                info = info.Trim();
            }
            else
            {
        ...
        public void CreateTextNote(Document doc, string text, XYZ centerP, Reference reference, double yOffset = 0)
        {
            ...
            // 计算偏移量，使对齐点为文本的下边中心
            XYZ offset = new XYZ(-textNoteWidth / 2, 1.25 + yOffset, 0);
