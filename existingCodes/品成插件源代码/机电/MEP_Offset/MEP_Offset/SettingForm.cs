...
namespace MEP_Offset
{
    public partial class SettingForm : Form
    {
        ...
        private void btn_OK_Click(object sender, EventArgs e)
        {
            ...
            if (rbtn_LR.Checked)
            {
                Resolver.Direction = DirectionList.LR;
                Command.rbtn_Direction = "偏移";
            }
            else
            {
                Resolver.Direction = DirectionList.TB;
                Command.rbtn_Direction = "升降";
            }
            // 类型
            if (rbtn_OneSide.Checked)
            {
                Resolver.Type = TypeList.OneSide;
                Command.rbtn_Type = "单侧";
            }
            else
            {
                Resolver.Type = TypeList.TwoSide;
                Command.rbtn_Type = "两侧";
            }
            // 角度
            if (rbtn_15.Checked)
            {
        ...
        private void SettingForm_Load(object sender, EventArgs e)
        {
            ...
            //方向
            if (Command.rbtn_Direction == "偏移")
            {
                rbtn_LR.Checked = true;
            }
            else
            {
                rbtn_TB.Checked = true;
            }
            //类型
            if (Command.rbtn_Type == "单侧")
            {
