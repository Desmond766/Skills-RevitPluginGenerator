...
namespace AddHanger
{
    class ResolverForDuct
    {
        ...
        // 构造函数
        public ResolverForDuct(ExternalCommandData commandData)
        {
        ...
        {
            wide_Duct = mep.get_Parameter(BuiltInParameter.RBS_CURVE_WIDTH_PARAM)?.AsDouble() ?? mep.get_Parameter(BuiltInParameter.RBS_CURVE_DIAMETER_PARAM).AsDouble();
            height_Duct = mep.get_Parameter(BuiltInParameter.RBS_CURVE_HEIGHT_PARAM)?.AsDouble() ?? mep.get_Parameter(BuiltInParameter.RBS_CURVE_DIAMETER_PARAM).AsDouble();
