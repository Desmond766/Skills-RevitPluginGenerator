...
namespace AddHanger
{
    class ResolverForPipe
    {
        ...
        // 构造函数
        public ResolverForPipe(ExternalCommandData commandData)
        {
        ...
        {
            diameter_Pipe = mep.get_Parameter(BuiltInParameter.RBS_PIPE_DIAMETER_PARAM).AsDouble();
            double insulationLayerThickness = mep.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS).AsDouble();
            ...
            foreach (Parameter para in new_Hanger.Parameters)
                if (para.Definition.Name == "管道公称直径")
                {
                    if (hanger is FamilyInstance familyInstance && familyInstance.Symbol.FamilyName.Contains("抗震"))
                        para.Set(diameter_Pipe + insulationLayerThickness * 2);
                    else para.Set(diameter_Pipe);
                    break;
                }
            ...
            foreach (Parameter para in new_Hanger.Parameters)
                if (para.Definition.Name == "b_底层管道底高")
                {
                    if (hanger is FamilyInstance familyInstance && familyInstance.Symbol.FamilyName.Contains("抗震"))
                    {
                        para.Set(height_Hanger - (diameter_Pipe / 2 + insulationLayerThickness));
                    }
                    else
                    {
                        para.Set(height_Hanger);
