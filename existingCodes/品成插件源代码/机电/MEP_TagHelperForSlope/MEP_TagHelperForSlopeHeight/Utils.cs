...
namespace MEP_TagHelperForSlopeHeight
{
    class Utils
    {
        ...
        #region 获得管线净高 考虑高低板
        public static double GetClearHeight(View3D view, Document doc, XYZ point_Do, ElementId id)
        {
            double distance = 0.0;
            //梁的计算点point_Do 
            ...
            ExclusionFilter exclusionFilter = new ExclusionFilter(new ElementId[] { id });
            LogicalAndFilter andFilter = new LogicalAndFilter(floorOrCeiling, exclusionFilter);

            //相交
            ReferenceIntersector referenceIntersector = new ReferenceIntersector(floorOrCeiling, FindReferenceTarget.All, view);
            //ReferenceIntersector referenceIntersector = new ReferenceIntersector(andFilter, FindReferenceTarget.All, view);
