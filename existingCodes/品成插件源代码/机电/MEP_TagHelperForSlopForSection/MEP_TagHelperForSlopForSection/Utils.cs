...
namespace MEP_TagHelperForSlopeForSection
{
    class Utils
    {
        ...
        #region 获得管线净高 考虑高低板
        public static double GetClearHeight(View3D view, Document doc, XYZ point_Do, MEPCurve mep)
        {
            double distance = 0.0;
            //梁的计算点point_Do 
            ...
            if (mep is Pipe pipe)
            {
                distance -= pipe.get_Parameter(BuiltInParameter.RBS_PIPE_OUTER_DIAMETER).AsDouble() / 2.0;
            }
            else if (mep is CableTray cableTray)
            {
                distance -= cableTray.get_Parameter(BuiltInParameter.RBS_CABLETRAY_HEIGHT_PARAM).AsDouble() / 2.0;
            }
            else if (mep is Duct duct)
            {
                var shape = duct.ConnectorManager.Connectors.Cast<Connector>().First().Shape;
                if (shape == ConnectorProfileType.Round)
                {
                    distance -= duct.get_Parameter(BuiltInParameter.RBS_CURVE_DIAMETER_PARAM).AsDouble() / 2.0;
                }
                else
                {
                    distance -= duct.get_Parameter(BuiltInParameter.RBS_CURVE_HEIGHT_PARAM).AsDouble() / 2.0;
                }
            }
            // 减去隔热层厚度
            double thickness = mep.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS)?.AsDouble() ?? 0;
            distance -= thickness;

            distance *= 304.8;
            distance = Math.Round(distance * 0.1, 0) * 10;
