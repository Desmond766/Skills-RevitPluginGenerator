...
namespace TagHelper
{
    ...
    public class Command : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            //    XYZ direction = (((element as MEPCurve).Location as LocationCurve).Curve as Line).Direction;

            //    if ( mepDirection.AngleTo(direction)<Math.PI * (5 / 180)  || mepDirection.AngleTo(direction) > Math.PI * (175 / 180))
            //    {
            ...
            //{
            //    TaskDialog.Show("Revit", "选择的管线不平行");
            //    return Result.Cancelled;
            //}
            ...
            //用户自定义参数
            double tagSpacing = 500 / 304.8;
            double headLeaderLength = 1000.0 / 304.8;

            //根据名称找标签族
            var tagFamilyName_RightAlign = "PC_管道标记_直径_标高_右对齐";
            var tagFamilyName_LeftAlign = "PC_管道标记_直径_标高_左对齐";

            ElementId tagTypeId_RightAlign = Utils.FindTypeIdByFamilyName(doc, tagFamilyName_RightAlign);
            ElementId tagTypeId_LeftAlign = Utils.FindTypeIdByFamilyName(doc, tagFamilyName_LeftAlign);
            ...
            FamilySymbol arrowDetialSymbol = doc.GetElement(Utils.FindTypeIdByFamilyName(doc, arrowDetialFamilyName)) as FamilySymbol;
