...
namespace AddHanger4GY
{
    class ResolverForGY
    {
        ...
        // 构造函数
        public ResolverForGY(ExternalCommandData commandData)
        {
        ...
        //方法
        public void Resolve(View3D view3D, Element hanger, Element mep1, Element mep2, XYZ location, MEPCurve minMep = null)
        {
            ...
            if (hanger is FamilyInstance familyInstance && familyInstance.Symbol.FamilyName.Contains("抗震"))
            {
                if (mep1 is Pipe) b += mep1.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS).AsDouble();
                if (mep2 is Pipe) c += mep2.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS).AsDouble();
            }
            ...
            if (hanger is FamilyInstance familyInstance1 && familyInstance1.Symbol.FamilyName.Contains("抗震") && mep1 is Pipe) 
                height_Hanger -= mep1.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS).AsDouble();

            // UPDATE: 25.12.18
            if (minMep != null && minMep.Id != mep1.Id)
            {
                height_Hanger = (minMep.Location as LocationCurve).Curve.Project(location).XYZPoint.DistanceTo(point_Hanger) - Utils.GetHalfHeight(minMep);
                if (hanger is FamilyInstance familyInstance2 && familyInstance2.Symbol.FamilyName.Contains("抗震") && minMep is Pipe)
                    height_Hanger -= minMep.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS).AsDouble();
            }
